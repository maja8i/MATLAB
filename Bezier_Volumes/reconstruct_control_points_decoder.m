...
for lvl = start_lvl:(max_lvl - 1)
    ...
    for occ_cell = 1:size(SpatialIndex{lvl}, 1)
        ...
        if (lvl >= pp_first_nonempty) && (post_pruning_array{lvl}(occ_cell) == 1)
            ...
            %do nothing further as this cell's control points have already
            %been reconstructed
            continue;  
        %If this cell is not a leaf (i.e., it is internal)
        else
            %Increment oc_code_cntr only when we find an internal cell at
        ...
        end %End check if (lvl >= pp_first_nonempty) && (post_pruning_array{lvl}(occ_cell) == 1) 
    end %End occ_cell
