    ...
    methods
        ...
        function obj = octreeClass(mortonCodes,depth)
            ...
            for level = 1:depth
                ...
                lastChildPtr = cumsum(obj.ChildCount{level});
                %obj.FirstChildPtr{level} = lastChildPtr - obj.ChildCount{level} + 1;
...
%                 fprintf('level=%d meanBucketSize=%g maxBucketSize=%d meanProbeCount=%g maxProbeCount=%d\n',level,meanBucketSize,maxBucketSize,meanProbeCount,probeCount);
        ...
        % Return occupancy codes concatenated in breadth-first order.
        function oC = occupancyCodes(obj)
            oC = uint8.empty;
            for level = 1:obj.Depth
