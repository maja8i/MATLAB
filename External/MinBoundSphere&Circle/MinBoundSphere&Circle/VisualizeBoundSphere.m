...
function H=VisualizeBoundSphere(X,R,C,col)  %Modified by Maja
...
if strcmpi(class(X),'TriRep')
    ...
    %H(1)=scatter3(X(:,1),X(:,2),X(:,3), 5, [col(:, 1), col(:, 2), col(:, 3)]);  %Modified by Maja
end
axis equal off
hold on
...
set(H(2),'EdgeColor','none','FaceColor','none','FaceAlpha',0.5) %Modified by Maja
