%Colour shifted voxels by using the colour of the nearest voxel in the
%original (unshifted) point cloud.
...
ptcloud_file_orig = [ptcloud_name '.ply'];
...
colours_orig = ptcloud_orig(I, 4:6);
...
for s = shifts
    %Read in reconstructed (shifted) point cloud
    if shift_dir == 0
        [plyStruct_recon, ptcloud_recon, ~] = plyRead([ptcloud_name '_Xshift' num2str(s) '.ply']);
    elseif shift_dir == 1
        [plyStruct_recon, ptcloud_recon, ~] = plyRead([ptcloud_name '_Yshift' num2str(s) '.ply']);
    elseif shift_dir == 2
        [plyStruct_recon, ptcloud_recon, ~] = plyRead([ptcloud_name '_Zshift' num2str(s) '.ply']);
    end
    %Get Morton codes for all x, y, z coordinates in the reconstructed
    %point cloud
    ...
    %Plot the reconstructed and recoloured point cloud
    a = [xyz_recon colours_recon];
    figure; 
    scatter3(a(:, 1), a(:, 2), a(:, 3), 5, [a(:, 4)./255, a(:, 5)./255, a(:, 6)./255], 'filled');
    axis equal; axis off;
    
    %Write the shifted and recoloured point cloud to PLY file ...
    plyStruct2 = plyStruct_recon;

    plyStruct2.propArrayListList = cell(1, 1);
    plyStruct2.propArrayListList{1}{1} = xyz_recon(:, 1);   %Shifted voxel x coordinates
    plyStruct2.propArrayListList{1}{2} = xyz_recon(:, 2);   %Shifted voxel y coordinates
    plyStruct2.propArrayListList{1}{3} = xyz_recon(:, 3);   %Shifted voxel z coordinates
    plyStruct2.propArrayListList{1}{4} = colours_recon(:, 1);   %R 
    plyStruct2.propArrayListList{1}{5} = colours_recon(:, 2);   %G 
    plyStruct2.propArrayListList{1}{6} = colours_recon(:, 3);   %B 
    ...
    %Save the shfited and recoloured point cloud in the current directory
    if shift_dir == 0
        plyWrite(plyStruct2, [ptcloud_name '_Xshift' num2str(s) '_recoloured.ply'], format);
    elseif shift_dir == 1
        plyWrite(plyStruct2, [ptcloud_name '_Yshift' num2str(s) '_recoloured.ply'], format);
    elseif shift_dir == 2
        plyWrite(plyStruct2, [ptcloud_name '_Zshift' num2str(s) '_recoloured.ply'], format);
    end
end %End s
