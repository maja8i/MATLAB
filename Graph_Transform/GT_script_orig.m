...
%quantization.  
q_stepsize = 16; %Comment this out if using various quantization step sizes
...
if (fixed_p_qstep == 1)
    ...
    %correct directories
    codec_name_rlgr = ['GT_Geom_QStep' num2str(q_stepsize) '_OTBlock' num2str(GT_block_lvl) '_RLGR'];
    codec_name_entropy = ['GT_Geom_QStep' num2str(q_stepsize) '_OTBlock' num2str(GT_block_lvl) '_ENTROPY'];
    %Define the output directories for each codec
    outputdir_rlgr = [codec_results_path ptcloud_name '\' vox_novox_dir '\' codec_name_rlgr '\'];
    ...
    newdir_stat_rlgr = mkdir(outputdir_rlgr); 
    if newdir_stat_rlgr == 1
        disp(['Created output directory: ' outputdir_rlgr]);
    else
        error(['ERROR: Could not create output directory ' outputdir_rlgr]);
    end
    ...
    %For RLGR bitrates
    fid_rlgr = fopen([outputdir_rlgr reconstruction_prefix '_geom_bitrates.txt']);
    if fid_rlgr ~= -1
        %Close the open file (must close before deleting)
        fclose(fid_rlgr);
        %Delete the existing file
        disp(['The file ' outputdir_rlgr reconstruction_prefix '_geom_bitrates.txt already exists. Deleting ...']);
        delete([outputdir_rlgr reconstruction_prefix '_geom_bitrates.txt']);  
    end
    %Open a new text file in append mode
    fid_rlgr = fopen([outputdir_rlgr reconstruction_prefix '_geom_bitrates.txt'], 'a');
    if fid_rlgr == -1
        error(['ERROR: Could not open text file ' outputdir_rlgr reconstruction_prefix '_geom_bitrates.txt']);
    else
        disp(['Successfully opened file ' outputdir_rlgr reconstruction_prefix '_geom_bitrates.txt']);
    end
    ...
    for p = 100:-10:10
        ...
        disp('------------------------------------------------------------');
        %Run the encoder
        [nbr_occ_voxels_vec, quantized_spectral_coeffs_cat, eig_indices_cat, XYZ_indices_cat, lone_voxels_cat, rlgr_bits, entropy_bits] = GT_encoder_orig(ptcloud_file, b, GT_block_lvl, p, q_stepsize);
        %Write the bpv (bits per reconstructed voxel, or point) and total 
        %bitrate to file, for the current compressed point cloud
        fprintf(fid_rlgr, '%f %f\r\n', rlgr_bits);
        fprintf(fid_entropy, '%f %f\r\n', entropy_bits);
        %Run the decoder
        [recon_xyz] = GT_decoder_orig(nbr_occ_voxels_vec, quantized_spectral_coeffs_cat, eig_indices_cat, XYZ_indices_cat, lone_voxels_cat, q_stepsize, p);
        recon_xyz_matrix = cell2mat(recon_xyz');    %N x 3 matrix
        plyStruct2.propArrayListList{1}{1} = recon_xyz_matrix(:, 1);   %X coordinates
        plyStruct2.propArrayListList{1}{2} = recon_xyz_matrix(:, 2);   %Y coordinates
        plyStruct2.propArrayListList{1}{3} = recon_xyz_matrix(:, 3);   %Z coordinates
        %Write the current reconstructed point cloud to file, in PLY format
        if (reconstruction_counter <= 9)
            plyWrite(plyStruct2, [outputdir_rlgr reconstruction_prefix '_distorted0' num2str(reconstruction_counter) '.ply'], format);
            plyWrite(plyStruct2, [outputdir_entropy reconstruction_prefix '_distorted0' num2str(reconstruction_counter) '.ply'], format);
        else
            plyWrite(plyStruct2, [outputdir_rlgr reconstruction_prefix '_distorted' num2str(reconstruction_counter) '.ply'], format);
...
elseif (fixed_p_qstep == 0)
    ...
    codec_name_rlgr = ['GT_Geom_p' num2str(p) '_OTBlock' num2str(GT_block_lvl) '_RLGR'];
    codec_name_entropy = ['GT_Geom_p' num2str(p) '_OTBlock' num2str(GT_block_lvl) '_ENTROPY']; 
    %Define the output directories for each codec
    outputdir_rlgr = [codec_results_path ptcloud_name '\' vox_novox_dir '\' codec_name_rlgr '\'];
    outputdir_entropy = [codec_results_path ptcloud_name '\' vox_novox_dir '\' codec_name_entropy '\'];
    %Create the directories corresponding to the above codec names
    ...
    if newdir_stat_rlgr == 1
        ...
        disp(['ERROR: Could not create output directory ' outputdir_rlgr]);
    end
    newdir_stat_entropy = mkdir(outputdir_entropy); 
    if newdir_stat_entropy == 1
        disp(['Created output directory: ' outputdir_entropy]);
